...
public class ProjectionCallHandlerTest {
    ...
    public void setUp() {
        ...
        mProjectionCallHandler =
                new ProjectionCallHandler(mContext, mTelecomManager, mCarProjectionManager);
    }
    ...
    public void emergencyCall_isNotSuppressed() {
        ...
        when(call.getDetails().getHandle()).thenReturn(Uri.parse("tel:911"));

        assertThat(mProjectionCallHandler.onTelecomCallAdded(call)).isFalse();
    }

    @Test
    public void nonEmergencyCall_isSuppressed() {
        sendProjectionStatus(suppressableStatus());

        Call call = createCall(mHfpAccount);
        when(call.getDetails().getHandle()).thenReturn(Uri.parse("tel:123"));

        assertThat(mProjectionCallHandler.onTelecomCallAdded(call)).isTrue();
    }

    @Test
    public void emergencyCallback_fromProperty_isNotSuppressed() {
        sendProjectionStatus(suppressableStatus());

        Call call = createCall(mHfpAccount);
        when(call.getDetails().hasProperty(Call.Details.PROPERTY_EMERGENCY_CALLBACK_MODE))
                .thenReturn(true);
    ...
    @Test
    public void emergencyCallback_fromWindow_isNotSuppressed() {
        sendProjectionStatus(suppressableStatus());
        ...
        Call call = createCall(mHfpAccount);
        when(call.getDetails().getExtras()).thenReturn(extras);
        android.util.Log.d("foo", call.getDetails().getExtras().toString());

        assertThat(mProjectionCallHandler.onTelecomCallAdded(call)).isFalse();
    }

    @Test
    public void emergencyCallback_outOfWindow_isSuppressed() {
        sendProjectionStatus(suppressableStatus());
